...
def query_pypi(package, include_prereleases):
    ...
    try:
        response = requests.get("https://pypi.org/pypi/{package}/json", timeout=1)
...
class UpdateResult:
    ...
    def __str__(self):
        ...
        retval = "Version {self.running_version} of {self.package_name} is outdated. Version {self.available_version} "
        if self.release_date:
            retval += "was released {pretty_date(self.release_date)}."
        else:
            retval += "is available."
        return retval
...
def pretty_date(the_datetime):
    ...
    if diff.days > 7 or diff.days < 0:
        ...
        return "{int(round(diff.seconds / 60))} minutes ago"
    elif diff.seconds < 7200:
        return "1 hour ago"
    else:
        return "{int(round(diff.seconds / 3600))} hours ago"
